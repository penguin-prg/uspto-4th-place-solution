...
if os.path.exists("/kaggle/working"):
    subprocess.run(
        "pip install /kaggle/input/whoosh-wheel-2-7-4/Whoosh-2.7.4-py2.py3-none-any.whl", shell=True
    )
...
def get_query_parser():
    ...
    xor_tagger = whoosh.qparser.OperatorsPlugin.OpTagger(
        " XOR ", BrsXorGroup, whoosh.qparser.InfixOperator
    )
    qp.replace_plugin(
        whoosh.qparser.OperatorsPlugin([(xor_tagger, 0)], Require=None, AndNot=None, AndMaybe=None)
    )
    return qp
